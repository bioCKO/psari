...
my $subject_overlap = 0; die "subject overlap isn't supported" if $subject_overlap;
...
sub sort_interval_q_sp { 
    ...
    ||
    $a->[1][0] <=> $b->[1][0] || $a->[1][1] <=> $b->[1][1]  # subject +
}
sub sort_interval_q_sm { 
    $a->[0][0] <=> $b->[0][0] || $a->[0][1] <=> $b->[0][1]  # query +
    ||
    $b->[1][1] <=> $a->[1][1] || $b->[1][0] <=> $a->[1][0]  # subject -
}
...
sub filter_hsps(@) {
    ...
    if ($overlap_filter eq "bits") {
        ...
        my @hsps_m;
        foreach (@hsps) { if ($_->{HIT_FRAME} > 0) { push @hsps_p, scalar($_); } else { push @hsps_m, scalar($_); } }
        sub sort_interval_b_sp {  # subject +, use reverse bits and 0 out loser if overlapping
            ...
            if (! are_disjoint($aint, $bint, 0) && $a->{BITS} != $b->{BITS}) {
                if ($a->{BITS} < $b->{BITS}) { $a->{BITS} = 0; } else { $b->{BITS} = 0; }  # 0 bits if low overlap
                $b->{BITS} <=> $a->{BITS}
            } else {
...
sub are_neighbors_p($$$) {  # intervals $x and $y are neighbors on +
    #print STDERR "*** are_neighbors_p didn't get 3 arguments\n" if (@_ != 3);
    my ($x, $y, $z) = @_; return (($$x[1] < ( $$y[0] + $z)) ? 1 : 0);
}
sub are_neighbors_m($$$) {  # intervals $x and $y are neighbors on -
    #print STDERR "*** are_neighbors_m didn't get 3 arguments\n" if (@_ != 3);
    my ($x, $y, $z) = @_; return (($$x[0] > ( $$y[1] - $z)) ? 1 : 0);
}
my $are_hsp_neighbors_pp = sub($$) { # hsps are neighbors on q + s +
    #print STDERR "*** are_hsp_neighbors_pp didn't get 2 arguments\n" if (@_ != 2);
    my ($x, $y) = @_;
    return ((are_neighbors_p($x->[0], $y->[0], $query_overlap) 
             && 
             are_neighbors_p($x->[1], $y->[1], $subject_overlap)) 
            ? 1 : 0);
};
my $are_hsp_neighbors_pm = sub($$) { # hsps are neighbors on q + s -
    #print STDERR "*** are_hsp_neighbors_pm didn't get 2 arguments\n" if (@_ != 2);
    my ($x, $y) = @_;
    return ((are_neighbors_p($x->[0], $y->[0], $query_overlap) 
             && 
             are_neighbors_m($x->[1], $y->[1], $subject_overlap)) 
            ? 1 : 0);
};
...
while ($result = $report->next_result) {
    ...
    while ($hit = $result->next_hit) {
        ...
        foreach (@hsps) {
            ...
            my @h = ( [ $_->start('query'), $_->end('query') ],     # [ 0 ][ 0 , 1 ]
                      [ $_->start('subject'), $_->end('subject') ], # [ 1 ][ 0 , 1 ]
                      ($_->{HIT_FRAME} > 0 ? "+" : "-"),            # [ 2 ]  frame
                      $_                                            # [ 3 ]  hsp object
                    );
            if      ($h[2] eq "+") {   push @hsps_p, \@h;
            } elsif ($h[2] eq "-") {   push @hsps_m, \@h;
        ...
        my $finalize_candidate = sub {
            ...
            for (my ($i,$j) = (0,1); $query_overlap and $j < @blocks; ++$i, $j=$i+1) {  # will not enter if one block
                ...
                if (! are_disjoint($bi->[0], $bj->[0], 0)) {
                    ...
                    my ($bic, $bjc) = (substr($bi->[3]->{HOMOLOGY_SEQ}, -$q_trim, $q_trim), 
                                       substr($bj->[3]->{HOMOLOGY_SEQ}, 0, $q_trim));
                    ...
                    if ($bic eq $bjc or $bjc =~ /[$blast_mismatch]/) { 
                        ...
                        if ($strand eq "+") {
                            $bi->[3]->{HIT_START} += $s_trim; $bj->[1][0] += $s_trim;
                        } elsif ($strand eq "-") {
                            $bi->[3]->{HIT_END} -= $s_trim; $bi->[1][1] -= $s_trim;
                        ...
                        if ($strand eq "+") {
                            ...
                            $bi->[3]->{HIT_START} += $s_trim; $bi->[1][0] += $s_trim;
                        } else { die "some mistake"; }
            ...
            $candidate->{source_aligned_length} = ($candidate->{source_interval}->[1] 
                                                   - $candidate->{source_interval}->[0] + 1 );
            ...
            $candidate->{candidate_length} = ( $candidate->{candidate_interval}->[1] 
                                               - $candidate->{candidate_interval}->[0] + 1 );
            ...
            $candidate->{candidate_frac_identical} = $candidate->{candidate_covered_frac_identical}
                                                     * $candidate->{candidate_frac_covered};
        ...
        my $create_candidates = sub {
            ...
            for (my $i = 0; $i < @hsps; ++$i) {
                ...
                if ($cand) {
                    ...
                    $cand->{candidate_name} = sprintf "%s:$candidate_number_format", 
                                                      $cand->{source_name},
                                                      $cand->{source_candidate_number}; 
                    $cand->{candidate_id} = $cand->{candidate_name};
                    my $block_num = 0;
                    foreach (@{$cand->{block_stats}}) {
                        ++$block_num;
                        $_->{block_name} = sprintf "%s:$candidate_block_number_format",
                                                   $cand->{candidate_name},
                                                   $block_num;
        ...
        my $write_candidate_gff = sub {
            ...
            push @parent_attr, sprintf "parentCov=%d-%d", $cand->{source_interval}->[0], 
                                                          $cand->{source_interval}->[1];
            ...
            print BED_intervals join("\t", $cand->{candidate_scaffold}, 
                                           $cand->{candidate_interval}->[0],
                                           $cand->{candidate_interval}->[1]), "\n" if $BED_intervals;
            ...
            foreach (@{$cand->{block_stats}}) {
                ...
                push @child_attr, sprintf "parentCov=%d-%d", $_->{block_source_interval}->[0], 
                                                             $_->{block_source_interval}->[1];
                ...
                print BED_intervals join("\t", $_->{block_candidate_scaffold}, 
                                               $_->{block_candidate_interval}->[0],
                                               $_->{block_candidate_interval}->[1]), "\n" if $BED_intervals;
